...
import { createAphroditeEngine } from './aphrodite-engine-provider';

const TEST_PROMPT: LanguageModelV1Prompt = [
    { role: 'user', content: [{ type: 'text', text: 'Hello' }] },
];

const provider = createAphroditeEngine({ apiKey: 'test-api-key' });
const model = provider.chat('aphrodite-engine-small-latest');

describe('doGenerate', () => {
    const server = new JsonTestServer(
        'https://127.0.0.1:2242/v1/completions',
    );
    ...
    function prepareJsonResponse({
        ...
        server.responseBodyJson = {
            ...
            created: 1711113008,
            model: 'aphrodite-engine-small-latest',
            choices: [
                {
                    index: 0,
                    message: {
    ...
    it('should pass custom headers', async () => {
        ...
        const provider = createAphroditeEngine({
            apiKey: 'test-api-key',
            headers: {
        ...
        await provider.chat('aphrodite-engine-small-latest').doGenerate({
            inputFormat: 'prompt',
            mode: { type: 'regular' },
            prompt: TEST_PROMPT,
        });
    ...
    it('should pass the api key as Authorization header', async () => {
        ...
        const aphroditeEngine = createAphroditeEngine({ apiKey: 'test-api-key' });

        await aphroditeEngine.chat('aphrodite-engine-small-latest').doGenerate({
            inputFormat: 'prompt',
            mode: { type: 'regular' },
            prompt: TEST_PROMPT,
        });
...
describe('doStream', () => {
    ...
    function prepareStreamResponse({ content }: { content: string[] }) {
        server.responseChunks = [
            ...
            `"created":1711097175,"model":"aphrodite-engine-small-latest","choices":[{"index":0,` +
    ...
    it('should stream tool deltas', async () => {
        server.responseChunks = [
            `data: {"id":"ad6f7ce6543c4d0890280ae184fe4dd8","object":"chat.completion.chunk","created":1711365023,"model":"aphrodite-engine-large-latest",` +
        ...
        const { stream } = await createAphroditeEngine({
            apiKey: 'test-api-key',
            generateId: () => 'test-id',
        })
            .chat('aphrodite-engine-large-latest')
            .doStream({
                inputFormat: 'prompt',
                mode: {
                    type: 'regular',
    ...
    it('should pass custom headers', async () => {
        ...
        await provider.chat('aphrodite-engine-small-latest').doStream({
            inputFormat: 'prompt',
            mode: { type: 'regular' },
            prompt: TEST_PROMPT,
        });
